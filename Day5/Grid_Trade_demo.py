...
def initialize(context):
    g.count = 30  # 每个行业初筛后的股票数量
    g.cash = 1000000  # 启动资金
    g.buy_stock = []  # 股票池
    g.initial_price = {}
    g.reselect = 3  # 重新更新股票池的周期（月）
    g.PE_MAX = 50  # 选股的最大市盈率
    g.finalselect = 5
    g.month = context.current_dt.month
    run_monthly(select_stock_by_industry, 1, 'open')
    # run_weekly(select_stock_by_industry, 1, 'open')
    # run_daily(select_stock_by_industry, 'open')
...
def select_stock_by_industry(context):
    ...
    if month % g.reselect != g.month % g.reselect:
        return
    industry_list = [
        'C27', 'C39', 'I63'
        # 'I64', 'I65',
    ...
    for industry_code in industry_list:
        ...
        # 选fundamental比较好的前30只,pe_ration<30，按market_cap降序排序。
        q = query(
        ...
        stocks = list(s1[s1 < (g.finalselect + 1)].index)
        for stock in stocks:
